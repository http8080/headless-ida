...
| `decompile-all --out <file> [--filter] [--split]` | 전체 함수 일괄 디컴파일 (--split: 함수별 개별 파일) |
| `stack-frame <addr\|name>` | 스택 프레임 레이아웃 (로컬 변수, 인자, 오프셋) |
| `switch-table <addr\|name>` | switch/jump 테이블 분석 |
...
| `profile run malware` | 악성코드 프로필: C2, 암호화, 안티 분석, 네트워크 API |
| `profile run vuln` | 취약점 프로필: 위험 함수 (memcpy, strcpy, sprintf, system) |
| `profile run firmware` | 펌웨어 프로필: 주변장치 (UART/SPI/GPIO), 프로토콜, 부트 |
...
| `rename-batch <file>` | CSV(주소,이름) 또는 JSON 파일에서 일괄 이름 변경 |
...
| `annotations export --out <file>` | 이름/주석/타입 JSON 내보내기 |
...
| `export-script --out <file>` | 재현 가능한 IDAPython 스크립트 생성 |
